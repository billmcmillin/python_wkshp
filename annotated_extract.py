	...
	with open('genetic_tools.csv', 'rb') as genetic_terms:
		with open('countries.csv', 'rb') as countries:
			...
			text =  readfile.read()
                        ...
                        # The `import csv` statement at the top of the script gives us access to reader functions that work on csv files
			...
			country_reader = csv.reader(countries)
		        ...
		        # Create empty arrays, one for each csv file	
			plant_terms = []
			tool_terms = []
			country_terms = []
                       
                        # Now we need to take all the values from the csv file and put them into arrays so we can work with the values.
...
# Recall that text contains the contents of readfile, our bibliographic data. We want to know the location of the first occurrence of the work 'Journal' and we'll store it in a variable called `firstindex` 
firstindex = text.find('Journal')
			...
			def inner_extractor(start):
				# If the vallue of start equals -1, do the following. When searching for text, if it's not found, many functions return -1
					...
					return ['', -1]
                                ...
                                # Within text (our bibliographic data), find the location of '{{', starting looking at the location 'start' 
                                # and store the value in a variable called startindex
				...
				endindex = text.find('}}', start)
                                ...
                                # Return an array with two vallues: all of the text starting from startindex plus 2 characters all the way 
                                # to endindex and the number of endindex
				return [text[startindex + 2 : endindex], endindex]
                                # That's it -- the reverting indent means we're at the end of the function

                        # Defining a function that takes a parameter called planttext
			def plant_inner_term_extractor(planttext):
                                # Call the lower() function on planttext, making all characters lowercase, add a blank space at the beginning,
                                # and assign it to a variable called text_to_search.
			...
			def extractor(textindex):
				index = textindex
                                ...
                                # `len()` is a function that returns the number of characters in a string. When we find 'Title' or run out 
                                # of text to search, store the index (-1 for not found) in a variable called title_index.
				title_index = text.find('Title', index, len(text))
                                
                                # Take the number from the previous step and send it to the inner_extractor function. 
                                # That function returns an array with a string (the title if found or an empty string if not) 
                                # and a number of the ending index or -1 if not found. Take this array and store it in a variable called title.
